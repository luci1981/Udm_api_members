...
def add_member():
    ...
    new_member_cur = db.execute('SELECT id, name, email, level FROM members WHERE name=?', [name])
    new_member_result = new_member_cur.fetchone()

    return jsonify({'member' : {'id' : new_member_result['id'],
                               'name' : new_member_result['name'],
                               'email' : new_member_result['email'],
                               'level' : new_member_result['level']}})
...
def delete_member(member_id):
    ...
    return jsonify({'messasge' : 'The member has been deleted!'})
